...
def quick_start(model, dataset, config_dict, save_model=True):
    ...
    for hyper_tuple in combinators:
        ...
        if best_test_upon_valid[val_metric] > best_test_value:
            ...
            logger.info('save_best_val_model')
            model_save_path = os.path.join(config['checkpoint_dir'], f'best_val_model.pth')
            trainer.save_model(model_save_path)
        idx += 1
        # Save the model to a pth file
        
        logger.info('saving_per_param_model')
        model_save_path = os.path.join(config['checkpoint_dir'], f'per_param_model')
        trainer.save_model(model_save_path)
